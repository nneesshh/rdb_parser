...
void
load_zipmap(rdb_parser_t *parser, const char *zm, rdb_kv_chain_t **vall, uint32_t *size)
{
	uint32_t len = 0;
    int klen, vlen;
    char *key, *val;
    ...
    while (!ZM_IS_END(zm)) {
		...
		key = nx_palloc(parser->pool, klen + 1);
		...
		val = nx_palloc(parser->pool, vlen + 1);
...
void
zipmap_dump(rdb_parser_t *parser, const char *s)
{
